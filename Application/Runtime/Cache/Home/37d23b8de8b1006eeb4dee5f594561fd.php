...
<head>
  <meta charset="UTF-8">
  <?php if(is_array($article)): $i = 0; $__LIST__ = $article;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$var): $mod = ($i % 2 );++$i;?><title><?php echo ($var["title"]); ?></title><?php endforeach; endif; else: echo "" ;endif; ?>
  ...
  <div class="container">
    <div class="main" id="main">
      ...
      <div class="articleStyle">
        <?php if(is_array($article)): $i = 0; $__LIST__ = $article;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$article): $mod = ($i % 2 );++$i;?><p class="post_time">发布于：<?php echo (date("Y-m-d H:i", $article["post_time"])); ?></p>
          <h2><a href="javascript:;"><?php echo ($article["title"]); ?></a></h2>
          <p><?php echo ($article["content"]); ?></p><?php endforeach; endif; else: echo "" ;endif; ?>
      </div>

      <!-- 多说评论框 start -->
      <div class="ds-thread" data-thread-key="<?php echo ($article["id"]); ?>" data-title="<?php echo ($article["title"]); ?>" data-url="<?php echo U('article/article', array(), '');?>/id/<?php echo ($article["id"]); ?>"></div>
      <!-- 多说评论框 end -->
  ...
  <div class="frame">
    ...
    <ul class="list list-s1" data-role="accordion">
        <?php if(is_array($navigation)): foreach($navigation as $k=>$nav): ?><li>
            ...
            <ul class="child-list">
              <?php $__FOR_START_30724__=0;$__FOR_END_30724__=count($nav['children:']);for($i=$__FOR_START_30724__;$i < $__FOR_END_30724__;$i+=1){ ?><li><a href="<?php echo U('articleList/articleList', array(), '');?>/id/<?php echo ($nav['children:'][$i]['id']); ?>"><?php echo ($nav['children:'][$i]['name']); ?>(<?php echo(count($nav['children:'][$i]['children:'])) ?>)</a> </li><?php } ?>
            </ul>
