...
<div class="nav-swf">
  ...
  <div class="nav" id="nav">
    <ul>
      ...
      <?php if(is_array($navigation)): $i = 0; $__LIST__ = $navigation;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nav): $mod = ($i % 2 );++$i; if($nav["pid"] == 0): ?><li>
            <a href="<?php echo U('articleList/articleList', array(), '');?>/id/<?php echo ($nav["id"]); ?>"><?php echo ($nav["name"]); ?></a>
  ...
  <div class="container">
    <div class="main" id="main">
      <?php if(is_array($articleList)): $i = 0; $__LIST__ = $articleList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><div class="articleStyle">
          <p class="post_time">发布于：<?php echo (date("Y-m-d H:i", $list["post_time"])); ?></p>
          <h2><a href="<?php echo U('article/article', array(), '');?>/id/<?php echo ($list["id"]); ?>" title="点击阅读全文"><?php echo ($list["title"]); ?></a></h2>
        ...
        <ul class="pagination">
          <li>
            <?php if($page['page'] == 1): ?><span>上一页</span>
              <?php else: ?>
              <a href="<?php echo U('articleList', '', '');?>/page/<?php echo ($page['page'] - 1); ?>/id/<?php echo ($list["cid"]); ?>">上一页</a><?php endif; ?>
          </li>
          <?php $__FOR_START_11644__=1;$__FOR_END_11644__=$page['page_count'] + 1;for($i=$__FOR_START_11644__;$i < $__FOR_END_11644__;$i+=1){ ?><li>
              <?php if($i == $page['page']): ?><span><?php echo ($page["page"]); ?></span>
                <?php else: ?>
                <a href="<?php echo U('articleList', '', '');?>/page/<?php echo ($i); ?>/id/<?php echo ($list["cid"]); ?>"><?php echo ($i); ?></a><?php endif; ?>
            </li><?php } ?>
          <li>
            <?php if($page['page'] >= $page['page_count']): ?><span>下一页</span>
              <?php else: ?>
              <a href="<?php echo U('articleList', '', '');?>/page/<?php echo ($page['page'] + 1); ?>/id/<?php echo ($list["cid"]); ?>">下一页</a><?php endif; ?>
          </li>
        </ul>
      </div>
  ...
  <div class="frame">
    ...
    <ul class="list list-s1" data-role="accordion">
        <?php if(is_array($navigation)): foreach($navigation as $k=>$nav): ?><li>
            <i class="arrow_carrot-2right" data-role="click-it"></i><a href="<?php echo U('articleList/articleList', array(''), '');?>/id/<?php echo ($nav["id"]); ?>"><?php echo ($nav["name"]); ?>(<?php echo(count($nav['children:'])) ?>)</a>
            <ul class="child-list">
              <?php $__FOR_START_27759__=0;$__FOR_END_27759__=count($nav['children:']);for($i=$__FOR_START_27759__;$i < $__FOR_END_27759__;$i+=1){ ?><li><a href="<?php echo U('articleList/articleList', array(), '');?>/id/<?php echo ($nav['children:'][$i]['id']); ?>"><?php echo ($nav['children:'][$i]['name']); ?>(<?php echo(count($nav['children:'][$i]['children:'])) ?>)</a> </li><?php } ?>
            </ul>
  ...
  <div class="frame">
    ...
    <ul class="list list-s2">
      <?php if(is_array($lastArticle)): $i = 0; $__LIST__ = $lastArticle;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$art): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('article/article', array(), '');?>/id/<?php echo ($art["id"]); ?>"><?php echo ($art["title"]); ?></a> </li><?php endforeach; endif; else: echo "" ;endif; ?>
    </ul>
  </div>
