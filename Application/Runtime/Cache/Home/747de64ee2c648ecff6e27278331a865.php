    ...
    <div class="article">
      <?php if (is_array($article)): $i = 0;
        ...
        if (count($__LIST__) == 0) : echo "";
          ...
          ++$i; ?><h2><a href="javascript:;"><?php echo($article["title"]); ?></a></h2>
          <p><?php echo($article["content"]); ?></p><?php endforeach; endif;
      else: echo "";endif; ?>
    </div>

    <!-- 多说评论框 start -->
    <div class="ds-thread" data-thread-key="<?php echo($article["cid"]); ?>"
         data-title="<?php echo($article["title"]); ?>"
         data-url="<?php echo U('article/getArticleByCid', array(), ''); ?>/id/<?php echo($article["cid"]); ?>"></div>
    <!-- 多说评论框 end -->
    ...
    <div class="frame">
      ...
      <ul class="list list-s1" data-role="accordion">
        <?php if (is_array($navigation)): foreach ($navigation as $k => $nav): ?>
          ...
          <ul class="child-list">
            <?php $__FOR_START_19052__ = 0;
            $__FOR_END_19052__ = count($nav['children:']);
            for ($i = $__FOR_START_19052__; $i < $__FOR_END_19052__; $i += 1) { ?>
              <li><a
                href="<?php echo U('article/getArticleByCid', array(), ''); ?>/id/<?php echo($nav['children:'][$i]['id']); ?>"><?php echo($nav['children:'][$i]['name']); ?></a>
              </li><?php } ?>
          </ul>
