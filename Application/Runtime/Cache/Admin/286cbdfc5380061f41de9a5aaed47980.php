        ...
        <div class="form-group">
          ...
          <select class="form-control" id="select" name="pid">
            ...
            <?php if (is_array($list)): $i = 0;
              ...
              else: foreach ($__LIST__ as $key => $data): $mod = ($i % 2);
                ++$i;
                if ($category['pid'] == $data['id']): ?>
                  <option value="<?php echo($data["id"]); ?>"
                          selected> <?php echo($data['html']); ?> <?php echo($data['name']); ?> </option>
                <?php elseif ($category['id'] != $data['id']): ?>
                <option
                  value="<?php echo($data["id"]); ?>"> <?php echo($data['html']); ?> <?php echo($data['name']); ?> </option><?php endif; endforeach; endif;
            else: echo "";endif; ?>
          </select>
        </div>
        <div class="form-group">
          <label for="select" class="control-label">排序ID:</label>
          <input type="text" id="sort_id" name="sort_id" class="form-control sort-id"
                 value="<?php echo($category["sort_id"]); ?>">
        </div>
        <div class="form-group">
